...
//public imagePicker: ImagePicker, public camera: Camera
  constructor(public navCtrl: NavController, public navParams: NavParams,private http:HttpClient,public actionSheetCtrl: ActionSheetController, public alertCtrl: AlertController, public camera: Camera,public imagePicker:ImagePicker) {
  }
  ...
  ionViewDidLoad() {
    ...
    this.http.post('/api/userinfo',{"userID":this.userID}).subscribe(data=>{
      ...
      this.name=this.infoData[0].userName;
      this.headerImage=this.infoData[0].picture;
      this.myDate=this.infoData[0].birthday;
      this.area=this.infoData[0].location;
      this.sex=this.infoData[0].sex;
    })

  }

  save(){
    this.http.post('/api/change/info',{"name":this.name,"birthday":this.myDate,"sex":this.sex,"area":this.area,"userID":this.userID}).subscribe(data=>{
      if(data['status']==1){
  ...
  openCamera() {   
    const options: CameraOptions = {     
      ...
      sourceType: this.camera.PictureSourceType.CAMERA ,  //是打开相机拍照还是打开相册选择  PHOTOLIBRARY : 相册选择选择 CAMERA : 拍照,
    } 

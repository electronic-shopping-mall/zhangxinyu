...
export class AddAddressPage {
  ...
  save(){
    if(this.name==undefined || this.phone==undefined || this.area==undefined || this.detail==undefined){
      ...
      this.http.post('/api/addAddress',{"userID":this.userID,"name":this.name,"phone":this.phone,"area":this.area,"detail":this.detail,"isDefault":this.isDefault}).subscribe(data=>{
        if(data['status']==1){
          console.log('插入新地址成功');
          this.navCtrl.getPrevious().data.addAddressData = [this.name,this.phone,this.area,this.detail];
          this.navCtrl.pop();
        }
      });
    }
